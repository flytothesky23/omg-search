...
\uC5F0\uACB0 \uC218 ${node.degree || 0} \xB7 PageRank ${Math.round((node.pageRank || 0) * 100)}`;
  ...
  computeForceLayout(nodes, edges, width, height) {
    ...
    const sortedCommunities = Array.from(communityCounts.entries()).sort((a, b) => b[1] - a[1] || a[0] - b[0]).map(([community]) => community);
    ...
    nodes.forEach((node, index) => {
      ...
      const nodeRadius = node.kind === "tag" ? 5 + Math.min(node.degree || 0, 35) * 0.12 : 5 + Math.sqrt(Math.max(0, node.pageRank || 0)) * 15 + Math.min(node.degree || 0, 40) * 0.08;
    ...
    for (let iter = 0; iter < 260; iter++) {
      ...
      for (const edge of visibleEdges) {
        ...
        const force = (dist - desired) * (edge.type === "wikilink" ? 45e-4 : 25e-4) * alpha;
      ...
      for (const item of positions.values()) {
        ...
        item.vx += (center.x - item.x) * 55e-4 * alpha;
        item.vy += (center.y - item.y) * 55e-4 * alpha;
        item.vx += (safeWidth / 2 - item.x) * 14e-4 * alpha;
        item.vy += (safeHeight / 2 - item.y) * 14e-4 * alpha;
        item.x += item.vx;
        item.y += item.vy;
        item.vx *= 0.72;
        item.vy *= 0.72;
  ...
  graphLabelPlacement(pos, width, height) {
